...
CONFIG_PATH = ROOT / "data" / "trip_config.json"
...
def section_title(title: str, subtitle: str | None = None) -> None:
    subtitle_html = f'<div class="small-muted">{escape(subtitle)}</div>' if subtitle else ""
    st.markdown(
        f"""
...
def render_overview(config: dict[str, Any]) -> None:
    ...
    for item in [
        ...
        st.markdown(f'<div class="risk-card">{escape(item)}</div>', unsafe_allow_html=True)
...
def render_timeline(config: dict[str, Any]) -> None:
    ...
    tabs = st.tabs([f"{day.get('day', 'Day')} · {day.get('date', '')}" for day in schedule])
    for tab, day in zip(tabs, schedule, strict=False):
        with tab:
            for index, (time_range, activity) in enumerate(day.get("items", []), start=1):
                st.markdown(
                    f"""
                    <div class="time-row">
...
def render_weather(config: dict[str, Any]) -> None:
    ...
    section_title("天气和降级决策", "把天气不确定性提前转成明确的执行、降级和取消条件。")
...
def render_cars(config: dict[str, Any]) -> None:
    ...
    section_title("两车分组和分工", "两辆车各自固定司机、副驾和物资责任人，减少临场混乱。")
...
def main() -> None:
    ...
    st.sidebar.caption("轻量网页版 · 数据来自 JSON 配置")
    ...
    st.sidebar.caption("数据来源：data/trip_config.json")
